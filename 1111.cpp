...
void freeMFD(MFD* mf[], int n);//释放一级目录表
void freeUFD(UFD* uf[], int n);//释放二级目录表
void freeUOF(UOF* of[], int n);//释放已打开的文件表
int main()
{
    MFD* mf[MFD_n];//一级目录表
...
void freeMFD(MFD* mf[], int n) {
    for (int i = 0; i < n; i++) {
        free(mf[i]);
    }
}

void freeUFD(UFD* uf[], int n) {
    for (int i = 0; i < n; i++) {
        free(uf[i]);
    }
}

void freeUOF(UOF* of[], int n) {
    for (int i = 0; i < n; i++) {
        free(of[i]);
    }
}
